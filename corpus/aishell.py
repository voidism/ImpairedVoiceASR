...
OFFICIAL_TXT_SRC = ['bznsyp-bopomo-lm-norm.txt']
...
transcripts_file = open('/workspace/toolkits/End-to-end-ASR-Pytorch/data/Aishell/data_aishell/transcript/full_transcript_bpmf.txt').readlines()
transcripts = {}
    ...
    def __init__(self, path, split, tokenizer, bucket_size, ascending=False):
        ...
        for s in split:
            ...
            for i in range(len(split_list)):
                if str(split_list[i]).split('/')[-1].split('.')[0] in transcripts:
                    new_list.append(split_list[i])
                else:
                    print(split_list[i], "Not Found")
                    uf += 1
            print("%d examples not found!"%uf)
            split_list = new_list
...
class BznsypTextDataset(Dataset):
