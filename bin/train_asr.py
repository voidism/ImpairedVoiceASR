...
class Solver(BaseSolver):
    ...
    def __init__(self, config, paras, mode):
        ...
        self.grad_accum = self.config['hparas']['grad_accum']
        self.finetune_first = self.config['hparas']['finetune_first']
    ...
    def exec(self):
        ...
        while self.step < self.max_step:
            ...
            for data in self.tr_set:
                ...
                accum = accum_count % (self.grad_accum) == 0.0
                if self.step == 0 or accum:
                    # Backprop
                    ...
                    grad_norm = self.backward(total_loss, accumulate=True)
                    if self.step == 1 or self.step % self.PROGRESS_STEP == 0:
                        self.progress('Tr stat | Loss - {:.2f} | Grad. Norm - {:.2f} | {}'
                                      .format(total_loss.cpu().item(), grad_norm, self.timer.show()))
                    accum_count += 1
