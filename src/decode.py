...
class BeamDecoder(nn.Module):
    ...
    def forward(self, audio_feature, feature_len):
        ...
        if self.asr.attention is not None:
        ...
        for t in range(max_output_len):
            for hypothesis in prev_top_hypothesis:
                ...
                if self.asr.attention is not None:
                    ...
                    # Transformer
                    decoder_input = asr_prev_token # transformer do not use external attention
                    cur_prob, d_state = self.asr.decoder(decoder_input, encode_feature, encode_len) # emb_decoder is not support with transformer decoder, so d_state is not used for now.
                ...
                if self.asr.attention is not None:
                    ...
                    prev_attn = self.asr.decoder.layers.decoder.layers[-1].src_attn.attn.cpu() if store_att else None
    ...
    def addTopk(self, topi, topv, decoder_state, att_map=None,
        ...
        for i in range(beam_size):
            ...
            if ctc_state is not None:
                ...
                idx = ctc_candidates.index(topi[i].item()) #if topi[i].item() in ctc_candidates else 0
                ctc_s = ctc_state[idx, :, :]
                ctc_p = ctc_prob[idx]
