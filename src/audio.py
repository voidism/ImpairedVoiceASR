...
def create_transform(audio_config, dv=False):
    ...
    specaug = audio_config.pop("specaug")

    transforms = [ExtractAudioFeature(feat_type, feat_dim, **audio_config)]

    if specaug and not dv:
        from src.specaugment import FrequencyMask, TimeMask
        transforms.append(FrequencyMask(max_width=10, use_mean=True))
        transforms.append(TimeMask(max_width=10, use_mean=True))
